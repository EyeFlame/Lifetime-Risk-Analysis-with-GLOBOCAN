...
# Function to create map with both color coding and selective pie charts
create_color_pie_map <- function(world_data, df_map, ratio_var, title, 
                                 selected_countries = c("Argentina", "Australia", "Brazil", "Canada", 
                                                        "China", "France", "Germany", "India", "Indonesia", 
                                                        "Italy", "Japan", "South Korea", "Mexico", "Russia", 
                                                        "Saudi Arabia", "South Africa", "Turkey", "UK", "USA")) {
  ...
  # Create pie chart data only for selected countries
  country_centroids <- get_country_centroids(world_data)
  pie_data <- country_centroids %>%
    left_join(df_map %>% select(region, all_of(ratio_var)), by = "region") %>%
    filter(!is.na(.data[[ratio_var]]) & region %in% selected_countries) %>%
    mutate(
      # Create pie chart proportions
      rectal_prop = .data[[ratio_var]],
      other_prop = 1 - .data[[ratio_var]],
      # Determine pie chart size based on country importance/size
      pie_size = case_when(
        region %in% c("China", "USA", "India", "Russia", "Brazil") ~ 5,  # Largest countries
        region %in% c("Japan", "Germany", "UK", "France", "Italy") ~ 4,  # Major developed countries
        TRUE ~ 3.5  # Other selected countries
      )
    )
  ...
  p <- ggplot() +
    ...
    labs(
      title = title,
      subtitle = "Colors show ratio ranges for all countries; Pie charts show exact proportions for major countries only",
      caption = "Source: GLOBOCAN 2022 | Pie charts: red=rectal cancer, blue=other colorectal cancers"
    )
  
  # Add pie charts with new fill scale (only for selected countries)
  ...
  # Add country labels for key countries (top 5 by size)
  key_labels <- pie_data %>% 
    filter(region %in% c("China", "USA", "Japan", "Germany", "India")) %>%
    mutate(label_text = paste0(region, "\n", round(.data[[ratio_var]], 3)))
  
  p <- p + geom_text(data = key_labels,
                     aes(x = long, y = lat - 12, label = label_text),
